...
import { addData, auth, getData } from "../firebase";
...
function Home() {
  ...
  const handleAddChannel = async () => {
    const channelName = prompt("Enter a new channer name");

    if (channelName) {
      try {
        const data = await addData("channels", { channelName });
      } catch (error) {
        console.error(error);
      }
    }
    ...
    <>
      ...
      <div className="flex h-screen">
        ...
        <div className="bg-[#2f3136] flex flex-col min-w-max">
          ...
          border-b border-gray-800 p-4 hover:bg-[#34373c]"
          >
